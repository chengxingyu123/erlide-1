%% TODO: show ets tables related to objects
%% 
-module(console_sys_wx).
...
init([Notebook, Parent]) ->
    SysInfo = console_backend:sys_info(),
    {Info, Stat} = info_fields(),
    Panel = wxPanel:new(Notebook),
    Sizer = wxBoxSizer:new(?wxHORIZONTAL),
    {FPanel0, _FSizer0, Fields0} =
	console_lib:display_info(Panel, console_lib:fill_info(Info, SysInfo)),
    {FPanel1, _FSizer1, Fields1} =
	console_lib:display_info(Panel, console_lib:fill_info(Stat, SysInfo)),
    ...
    Timer = console_lib:start_timer(10),
...
create_sys_menu(Parent) ->
    ...
    object_console:create_menus(Parent, [View]).

update_syspage(#sys_wx_state{node = Node, fields=Fields, sizer=Sizer}) ->
    SysInfo = object_console:try_rpc(Node, console_backend, sys_info, []),
    {Info, Stat} = info_fields(),
    console_lib:update_info(Fields, console_lib:fill_info(Info, SysInfo) ++
				 console_lib:fill_info(Stat, SysInfo)),
...
info_fields() ->
    Info = [{"System and Architecture",
	    ...
	    {"Objects Store Usage",
	     [{"Total Objects", number_of_objects},
	      {"Total Facts", number_of_facts},
		  {"Total in Running",number_of_running},
		  {"Total in Waiting",number_of_waiting},
		  {"Total in Start",number_of_start}
	     ]}
	   ],
...
handle_info(refresh_interval, #sys_wx_state{panel = Panel,
	...
	update_syspage(State)
	    ...
	    object_console:return_to_localnode(Panel, Node)
    end,
    {noreply, State};
...
handle_info({active, Node}, #sys_wx_state{parent = Parent, panel = Panel,
	...
	{noreply, UpdState#sys_wx_state{timer=console_lib:start_timer(Timer)}}
    catch error:{badrpc, _} ->
	    object_console:return_to_localnode(Panel, Node),
	    {noreply, State}
    end;
...
handle_info(not_active, #sys_wx_state{timer = Timer} = State) ->
    {noreply, State#sys_wx_state{timer = console_lib:stop_timer(Timer)}};
...
handle_event(#wx{id = ?ID_REFRESH_INTERVAL,
    ...
    Timer = console_lib:interval_dialog(Notebook, Timer0, 1, 5*60),

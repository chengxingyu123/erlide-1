...
%%		 {"Time", ?wxLIST_FORMAT_CENTRE, 50}, 
		 {"Class", ?wxLIST_FORMAT_LEFT, 120},
		 {"Memory", ?wxLIST_FORMAT_RIGHT, 80},
		 {"Attributes",  ?wxLIST_FORMAT_RIGHT, 350},
...
init_table_holder(Parent, Attrs) ->
    Backend = spawn_link(node(), console_backend,object_collect,[self()]),
...
table_holder(#holder{info=#etop_info{procinfo=Info}, attrs=Attrs,
	...
	refresh ->
	    Pid = spawn_link(Node,console_backend,object_collect,[self()]),
...
col_to_element(?COL_) -> #etop_proc_info.class;
